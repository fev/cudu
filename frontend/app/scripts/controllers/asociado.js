    ...
    .controller('AsociadoCtrl', ['$scope', '$routeParams', '$location', '$window', '$filter', 'Asociado', 'Grupo', 'Usuario', 'EstadosFormulario', 'Traducciones', 'Notificaciones', 'Ficha',
        function ($scope, $routeParams, $location, $window, $filter, Asociado, Grupo, Usuario, EstadosFormulario, Traducciones, Notificaciones, Ficha) {
            ...
            Ficha.queryAll(0, function (data) {
                $scope.fichas = _.filter(data, function (f) {
                    return f.tipoFicha == 0 && f.tipoEntidad == 0;
                });
                $scope.autorizaciones = _.filter(data, function (f) {
                    return f.tipoFicha == 1 && f.tipoEntidad == 0;
                });
            }, function () {
            });
            ...
            $scope.ignoreAccents = function(busqueda) {
                return function(item) {
                    if (!busqueda) return true;
                    var nombre = removeAccents(item.nombre.toLowerCase());
                    var apellidos = removeAccents(item.apellidos.toLowerCase());
                    var search = removeAccents(busqueda.toLowerCase());
                    return nombre.indexOf(search) > -1 || apellidos.indexOf(search) > -1;
                };
            };
            ...
            $scope.editar = function (id) {
                ...
                document.getElementById("panelEdicion").hidden = false;
                document.getElementById("logo-bienvenida").hidden = true;
                $scope.posAsociado = _.findIndex($scope.asociados, function (a) {
            ...
            $scope.guardar = function (id) {
                ...
                if (restricciones.noPuedeEditarOtrasRamas) {
                    ...
                    if (!mismasRamas) {
                        ...
                        var ramas = [];
                        if (u.ramaColonia) {
                            ramas.push(Traducciones.texto('rama.colonia'));
                        }
                        if (u.ramaManada) {
                            ramas.push(Traducciones.texto('rama.manada'));
                        }
                        if (u.ramaExploradores) {
                            ramas.push(Traducciones.texto('rama.exploradores'));
                        }
                        if (u.ramaExpedicion) {
                            ramas.push(Traducciones.texto('rama.expedicion'));
                        }
                        if (u.ramaRuta) {
                            ramas.push(Traducciones.texto('rama.ruta'));
                        }
                        $scope.erroresValidacion = [{
                ...
                guardar({id: id}, $scope.asociado).$promise.then(function (asociadoGuardado) {
                    ...
                    if (!asociadoNuevo) {
                        var pos = _.findIndex($scope.asociados, function (a) {
                            return a ? a.id === id : false;
                        });
            ...
            $scope.eliminar = function (id) {
                ...
                Asociado.delete({id: id}, function () {
                    _.remove($scope.asociados, function (a) {
                        return a ? a.id === id : false;
                    });
                    $scope.asociado = {};
                }, function () {
            ...
            $scope.marcar = function (asociado, e) {
                if (!asociado.marcado) {
                    ...
                    _.remove($scope.marcados, function (i) {
                        return i === asociado.id;
                    });
                }
            };
            ...
            $scope.establecerEstiloGuardado = function () {
                if ($scope.estado === EstadosFormulario.LIMPIO) {
                    return 'btn-default';
                }
                if ($scope.estado === EstadosFormulario.GUARDANDO) {
                    return 'btn-progress';
                }
                if ($scope.estado === EstadosFormulario.OK) {
                    return 'btn-success';
                }
                if ($scope.estado === EstadosFormulario.ERROR) {
                    return 'btn-error';
                }
                if ($scope.estado === EstadosFormulario.VALIDACION) {
                    return 'btn-warning';
                }
                return 'btn-default';
            };
            ...
            $scope.eliminarCargo = function (cargoId) {
                Asociado.eliminarCargo({id: $scope.asociado.id, cargoId: cargoId}, function () {
                    var pos = _.findIndex($scope.asociado.cargos, function (c) {
                        return c.id === cargoId;
                    });
            ...
            $scope.imprimirVisibles = function () {
                var f = $filter('filter');
                var visibles = f(f($scope.asociados, $scope.busqueda), function (a) {
                    return $scope.filtrar(a);
                });
