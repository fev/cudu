...
public interface CursoRepository extends PagingAndSortingRepository<Curso, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "insert into curso_participante (curso_id, asociado_id) values (:cursoId, :asociadoId);", nativeQuery = true)
    void añadirParticipante(@Param("cursoId") int cursoId, @Param("asociadoId") int asociadoId);
