...
public interface AsociadoRepository extends PagingAndSortingRepository<Asociado, Integer>, JpaSpecificationExecutor, AsociadoRepositoryCustom {
    ...
    Page<Asociado> findByGrupoIdOrderByActivoDesc(@Param("id") String id, Pageable pageable);
    ...
    @Query(value = "SELECT new org.scoutsfev.cudu.domain.dto.FormadorDto(F.id, F.nombreCompleto, F.telefono, F.email, F.nombreGrupo) FROM MiembroEscuelaDto F WHERE lower(F.nombreCompleto) LIKE :texto% AND F.cargoId = 34")
